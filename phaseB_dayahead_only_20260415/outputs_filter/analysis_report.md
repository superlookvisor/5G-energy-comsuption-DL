...
## 4. 结果摘要
- 最佳日前（按完整 24h 轨迹 MAPE）：`historical_proxy` + `RandomForest`，MAPE=0.1206。
- 负载单调分箱比例 100.00%，Spearman=0.6883；`p_base` 与 `n_cells` 相关 0.8024。
- ES 模式均值差（Top 3）：  
- S_ESMode5: -14.4606
- S_ESMode1: -12.4105
- S_ESMode2: -12.4100
- 各步 MAE 最优单步：h=24，`historical_proxy` + `RandomForest`。
