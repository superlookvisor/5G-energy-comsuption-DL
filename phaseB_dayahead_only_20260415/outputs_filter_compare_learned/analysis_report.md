...
```json
{
  ...
  "n_bs_after": 818,
  "n_rows_after": 90621,
  "output_subdir": "outputs_filter_compare_learned",
...
## 4. 结果摘要
- 最佳日前（按完整 24h 轨迹 MAPE）：`two_stage_proxy` + `RandomForest`，MAPE=0.0910。
- 负载单调分箱比例 100.00%，Spearman=0.6883；`p_base` 与 `n_cells` 相关 0.8080。
- ES 模式均值差（Top 3）：  
- S_ESMode5: -14.1767
- S_ESMode2: -13.4676
- S_ESMode1: -13.4674
...
## 5. 输出目录
本报告与 CSV/图位于同一目录：`outputs_filter_compare_learned/`。
