...
动态能耗随负载显著增加。负载分箱后的均值曲线中，相邻分箱单调不减比例为 100.00%，Spearman相关系数为 0.8476。同时，二次趋势线表明中高负载区间存在明显非线性，简单线性模型不足以完整描述该过程。

### 3.2 Day-ahead vs Intra-day Comparison
最佳日前模型为 `two_stage_proxy + RandomForest`，平均 MAPE=0.0924，峰值误差=4.5341，谷值误差=1.4335。最佳日内模型为 `RandomForest`，对应 1 步预测，MAE=1.8613，RMSE=2.9535，R²=0.9553。
...
- S_ESMode2: 负载分层后能耗差 -7.9143
- S_ESMode1: 负载分层后能耗差 -7.6782
- S_ESMode3: 负载分层后能耗差 -1.9929
...
- `P_base` 与小区数保持正相关，相关系数为 0.8074，说明阶段A与阶段B的分解是一致的。
