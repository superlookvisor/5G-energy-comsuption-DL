...
- `P_base` 继承自阶段A，采用 `D_relaxed + quantile_10`；直接观测基站 809 个，静态回填基站 114 个。
...
最佳日前模型为 `two_stage_proxy + RandomForest`，平均 MAPE=0.1100，峰值误差=5.8322，谷值误差=2.5376。最佳日内模型为 `RandomForest`，对应 1 步预测，MAE=1.8610，RMSE=2.9484，R²=0.9554。

日内滚动精度优于日前任务，因为其可直接观察当前真实 `load/ES/Energy`；日前任务只能依赖历史代理或两阶段代理，对峰值负载和ES切换的捕捉更弱。由于满足完整24小时连续轨迹的日前样本较少，补充的 horizon 级结果中最优单步 MAE 出现在 h=6，对应 two_stage_proxy + RandomForest。

### 3.3 Impact of ES Modes
ES模式的节能效果存在差异。依据负载分层后的均值差，能耗抑制更明显的模式为：
- S_ESMode5: 负载分层后能耗差 -14.1729
- S_ESMode2: 负载分层后能耗差 -13.4492
- S_ESMode1: 负载分层后能耗差 -13.1630
...
- `P_base` 与小区数保持正相关，相关系数为 0.8073，说明阶段A与阶段B的分解是一致的。
