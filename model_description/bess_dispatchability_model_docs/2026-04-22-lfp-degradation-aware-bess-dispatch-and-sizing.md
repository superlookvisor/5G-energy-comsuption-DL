...
给定候选容量集合 \(\mathcal{E}\)（例如 \(\{0,5,10,\dots,E_{\max}\}\) kWh），对每个 \(E\in\mathcal{E}\) 解内层调度问题得到日成本 \(J(E)\)，并选择：
...
在给定 \(E\) 与其他参数后，内层问题构造成线性规划（LP），可用 `cvxpy` / `pyomo` / `pulp` 等求解器稳定求解。
